				...
				if (c1.valutaPuntate(puntateArray)) {
					...
					giocatori.forEach((g) -> {
						if (g.getPuntata() > 0 && g.getNumeroGiocato() == dadoVincente) {
							...
							System.out.println("Il giocatore: " + g.getNome() + " ha puntato " + g.getPuntata()
									+ " ed ha vinto " + (g.getPuntata() * Dado.getRICARICO()) + " monete");
							g.reset();
						} else if (g.getPuntata() > 0) {
							c1.incasso(g.getPuntata());
							System.out.println(
									"Il giocatore: " + g.getNome() + " ha puntato " + g.getPuntata() + " ed ha perso ");
							g.reset();
						}
					});
				} else {
